...
def main(args):
    ...
    backbone = get_model(
        '3dpcnet_reconst_multiclass_classifMLP', encoder_name=cfg.network, img_size=cfg.img_size, dropout=0.0, fp16=cfg.fp16, num_features=cfg.embedding_size).cuda()
    ...
    margin_loss_attacktype = CombinedMarginLoss(
        # 64,
        cfg.scale,
        cfg.margin_list[0],
        cfg.margin_list[1],
    ...
    if cfg.optimizer == "sgd":
        ...
        module_partial_fc_attacktype = PartialFC_V2(
            # margin_loss, cfg.embedding_size, cfg.num_classes,
            margin_loss, cfg.embedding_size, 4,
            cfg.sample_rate, False)
        module_partial_fc_attacktype.train().cuda()

        # TODO the params of partial fc must be last in the params list
        opt = torch.optim.SGD(
            params=[{"params": backbone.parameters()}, {"params": module_partial_fc.parameters()}, {"params": module_partial_fc_attacktype.parameters()}],
    ...
    class_loss_attacktype_am = AverageMeter()
    ...
    for epoch in range(start_epoch, cfg.max_epoch):
        ...
        for batch_idx, (img, true_pointcloud, local_labels, local_labels_multiclass) in enumerate(train_loader):   # Bernardo
            ...
            module_partial_fc_attacktype.train()   # Bernardo
            ...
            class_loss_attacktype, probabilities_attacktype, pred_labels_attacktype = module_partial_fc_attacktype(face_embedd, local_labels_multiclass)     # Bernardo

            class_loss = cfg.lamb_class_loss * class_loss
            # class_loss_attacktype = cfg.lamb_class_loss_attacktype * class_loss_attacktype
            total_loss = class_loss_attacktype + reconst_loss + class_loss
            ...
            class_loss_attacktype_am.update(class_loss_attacktype.item(), 1)
            total_loss_am.update(total_loss.item(), 1)

            train_evaluator.update(pred_labels, local_labels, probabilities[:,1], pred_labels_attacktype, local_labels_multiclass, probabilities_attacktype)
            ...
            if (epoch % 2 == 0 or epoch == cfg.max_epoch-1) and batch_idx == 0:
                ...
                save_sample(path_dir_samples, img, true_pointcloud, local_labels,
                            ...
                            pred_labels_attacktype, local_labels_multiclass, probabilities_attacktype)
        print('')
        ...
        with torch.no_grad():
            ...
            callback_logging(global_step, reconst_loss_am, class_loss_attacktype_am, class_loss_am, total_loss_am, train_evaluator,
                             epoch, cfg.fp16, lr_scheduler.get_last_lr()[0], amp)
            reconst_loss_am.reset()
            class_loss_am.reset()
            class_loss_attacktype_am.reset()
            total_loss_am.reset()
            ...
            checkpoint = {
                ...
                "state_dict_softmax_fc_attacktype": module_partial_fc_attacktype.state_dict(),
                "state_optimizer": opt.state_dict(),
                "state_lr_scheduler": lr_scheduler.state_dict()
            }

            validate(chamfer_loss, module_partial_fc, module_partial_fc_attacktype, backbone, val_loader, val_evaluator,
...
# Bernardo
def validate(chamfer_loss, module_partial_fc, module_partial_fc_attacktype, backbone, val_loader, val_evaluator,
    ...
    with torch.no_grad():
        ...
        val_class_loss_attacktype_am = AverageMeter()
        val_total_loss_am = AverageMeter()
        for val_batch_idx, (val_img, val_pointcloud, val_labels, val_labels_multiclass) in enumerate(val_loader):
            ...
            val_loss_attacktype, val_probabilities_attacktype, val_pred_labels_attacktype = module_partial_fc_attacktype(val_face_embedd, val_labels_multiclass)     # Bernardo

            val_loss_class = cfg.lamb_class_loss*val_loss_class
            val_total_loss = val_loss_attacktype + val_loss_reconst + val_loss_class

            val_reconst_loss_am.update(val_loss_reconst.item(), 1)
            val_class_loss_am.update(val_loss_class.item(), 1)
            val_class_loss_attacktype_am.update(val_loss_attacktype.item(), 1)
            val_total_loss_am.update(val_total_loss.item(), 1)

            val_evaluator.update(val_pred_labels, val_labels, val_probabilities[:,1], val_pred_labels_attacktype, val_labels_multiclass, val_probabilities_attacktype)
            ...
            if (epoch % 10 == 0 or epoch == cfg.max_epoch-1) and val_batch_idx == 0:
                ...
                save_sample(path_dir_samples, val_img, val_pointcloud, val_labels,
                            ...
                            val_pred_labels_attacktype, val_labels_multiclass, val_probabilities_attacktype)
        print('')

        metrics = val_evaluator.evaluate()

        print('Validation:    val_ReconstLoss: %.4f    val_AttackClassLoss: %.4f    val_ClassLoss: %.4f    val_TotalLoss: %.4f    val_acc: %.4f%%    val_AUC: %.4f%%    val_apcer: %.4f%%    val_bpcer: %.4f%%    val_acer: %.4f%%' %
              (val_reconst_loss_am.avg, val_class_loss_attacktype_am.avg, val_class_loss_am.avg, val_total_loss_am.avg, metrics['acc'], metrics['auc_roc'], metrics['apcer'], metrics['bpcer'], metrics['acer']))

        writer.add_scalar('loss/val_reconst_loss', val_reconst_loss_am.avg, epoch)
        writer.add_scalar('loss/val_attackclass_loss', val_class_loss_attacktype_am.avg, epoch)
        ...
        val_class_loss_attacktype_am.reset()
...
def save_sample(path_dir_samples, img, true_pointcloud, local_labels, pred_pointcloud, pred_labels, pred_probs,
    ...
    for i in range(img.size(0)):
        sample_dir = f'sample={i}_true-label={local_labels[i]}_pred-label={pred_labels[i]}_pred-prob={pred_probs[i]}__true-label-attack={local_labels_multiclass[i]}_pred-labels-attack={pred_labels_attacktype[i]}_pred-prob={probabilities_attacktype[i]}'
