...
def main(args):
    ...
    backbone = get_model(
        '3dpcnet_reconst_classifMEANZ', img_size=cfg.img_size, dropout=0.0, fp16=cfg.fp16, num_features=cfg.embedding_size).cuda()
    ...
    for epoch in range(start_epoch, cfg.max_epoch):
        ...
        with torch.no_grad():
            ...
            if args.monitor_test:
                test(chamfer_loss, backbone, test_loader, test_evaluator,
...
def validate(chamfer_loss, backbone, val_loader, val_evaluator,
    ...
    with torch.no_grad():
        ...
        print('Validation:    val_ReconstLoss: %.4f    val_acc: %.4f%%    val_apcer: %.4f%%    val_bpcer: %.4f%%    val_acer: %.4f%%' %
              (val_reconst_loss_am.avg, metrics['acc'], metrics['apcer'], metrics['bpcer'], metrics['acer']))
...
# Bernardo
def test(chamfer_loss, backbone, test_loader, test_evaluator,
    ...
    with torch.no_grad():
        ...
        for test_batch_idx, (test_img, test_pointcloud, test_labels) in enumerate(test_loader):
            ...
            test_pred_pointcloud, val_pred_means_z, test_pred_labels = backbone(test_img)
