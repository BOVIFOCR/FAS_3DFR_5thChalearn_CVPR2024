...
class _3DPCNet_Reconst_ClassifMLP(nn.Module):
    ...
    def forward(self, img):
        ...
        def _forward(img):
            face_embedd, pred_pc = _regress_pointcloud(img)
            logits = _get_logits(pred_pc)
            return face_embedd, pred_pc, logits
...
class _3DPCNet_Reconst_Multiclass_ClassifMLP(nn.Module):
    def __init__(self, encoder_name='r18', face_embedd_size=256, num_output_points=2500, num_axis=3, fp16=False, **kwargs):
        super(_3DPCNet_Reconst_Multiclass_ClassifMLP, self).__init__()
        self.encoder_name = encoder_name
        self.face_embedd_size = face_embedd_size
        self.num_output_points = num_output_points
        self.num_axis = num_axis
    ...
    def forward(self, img):
        def _regress_pointcloud(img):        # input     -> x.shape = (batch, 3, 224, 224)
            ...
            embedd = embedd.unsqueeze(1)     # unsqueeze -> x.shape = (batch, 1, 256)
            pred_pc = self.decoder(embedd)   # decoder   -> x.shape = (batch, 2500, 3)
            return embedd.squeeze(), pred_pc

        def _get_logits_pc(x):
            x = x.reshape(x.size(0), self.num_output_points*self.num_axis)
            logits = self.classifier(x)
            return logits
        ...
        def _forward(img):
            face_embedd, pred_pc = _regress_pointcloud(img)
            logits_pc = _get_logits_pc(pred_pc)
            return face_embedd, pred_pc, logits_pc
...
class _3DPCNet_Reconst_ClassifMEANZ(nn.Module):
    def __init__(self, encoder_name='r18', face_embedd_size=256, num_output_points=2500, num_axis=3, fp16=False, **kwargs):
        super(_3DPCNet_Reconst_ClassifMEANZ, self).__init__()
        self.encoder_name = encoder_name
        self.face_embedd_size = face_embedd_size
        self.num_output_points = num_output_points
        self.num_axis = num_axis
    ...
    def get_decoder_Conv1x1(self, input_shape=(1, 256), output_shape=(1, 3, 2500)):
        decoder=nn.Sequential(nn.Conv1d(256,129,kernel_size=1), nn.ReLU(), nn.Conv1d(129,3,kernel_size=1), nn.Tanh())
        return decoder
    ...
    def get_classifier(self, num_points=2500, num_axis=3, embedding_size=128):
        class _3DPCNet_classifier:
            ...
            def __call__(self, pc):
                ...
                # sys.exit(0)
                mean_z = torch.mean(pc[:, :, 2], dim=1)  # mean of axis Z
                pred_label = torch.gt(mean_z, self.threshold).int()
                return mean_z, pred_label
        
        classifier = _3DPCNet_classifier(threshold=0.25)
        return classifier
    ...
    def forward(self, img):
        def _regress_pointcloud(img):        # input     -> x.shape = (batch, 3, 224, 224)
            embedd = self.encoder(img)       # encoder   -> x.shape = (batch, 256)
            # embedd = embedd.unsqueeze(1)     # unsqueeze -> x.shape = (batch, 1, 256)
            # embedd = torch.permute(embedd, (0, 2, 1))
            # print('embedd.size():', embedd.size())
            pred_pc = self.decoder(embedd)   # decoder   -> x.shape = (batch, 2500, 3)
            # print('pred_pc.size():', pred_pc.size())
            pred_pc = pred_pc.reshape(pred_pc.size()[0], 2500, 3)
            return pred_pc

        def _classify(x):
            # x = x.reshape(x.size(0), self.num_output_points*self.num_axis)
            pred_mean_z, pred_label = self.classifier(x)
            return pred_mean_z, pred_label

        def _forward(img):
            pred_pc = _regress_pointcloud(img)
            pred_mean_z, pred_label = _classify(pred_pc)
            return pred_pc, pred_mean_z, pred_label
